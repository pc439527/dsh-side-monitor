...
- **右侧 Drawer**：可拖动宽度的右侧面板（默认 480px，范围 320–900px），暗色科技风，与 DSH 主题一致；采用 **Container Query** 按面板自身宽度自适应（而非浏览器 viewport）。
- **三个模块**：
  - **概览**：CPU / 内存 / 网络吞吐 / 根分区磁盘 / 负载(1m/5m/15m) / Uptime / 系统信息 / 网络接口 / Docker 汇总。
  - **进程**：PID / 进程名 / 用户 / CPU% / 内存% / 命令，支持按 CPU、内存排序与关键字搜索；宽面板用表格，窄面板自动切换为进程卡片。
  - **Docker**：容器名 / 镜像 / 状态 / 运行时长 / CPU% / 内存 / 端口映射（Chip），含汇总计数；宽面板用表格，窄面板自动切换为容器卡片。
- **只读优先**：第一版不做任何控制操作。
