...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    //  - 0.0.0.0 / :: (any), 127.0.0.1 / ::1 (loopback) and empty resolve to
    //    the address the DSH GUI was reached on (window.location.hostname).
    // IPv6 literals are returned bracketed so they can go straight into a URL.
    function portHost(p) {
      var ip = p && p.hostIp ? String(p.hostIp).trim() : "";
      if (ip) {
        if (ip.indexOf(":") >= 0) {
          var isAny6 = ip === "::" || ip === "0:0:0:0:0:0:0:0";
          var isLoop6 = ip === "::1" || ip === "0:0:0:0:0:0:0:1";
          if (!isAny6 && !isLoop6) return "[" + ip + "]";
        } else {
          if (ip !== "0.0.0.0" && ip.indexOf("127.") !== 0) return ip;
        }
      }
      return window.location.hostname || "localhost";
    }

    // A port is "web" only when published AND a known web port on either side.
    function portWebProtocol(p) {
      if (!isPublishedPort(p)) return null;
    ...
    function DockerPanel(props) {
      ...
      function openPort(p, protocol) {
        if (!isPublishedPort(p)) return; // unpublished → never open
      ...
      function handlePortClick(p) {
        if (!isPublishedPort(p)) return; // unpublished → 禁止打开
