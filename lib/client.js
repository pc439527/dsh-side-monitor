 ...
 *
 * v0.2: the drawer is container-query driven — the overview metrics grid flows
 * 1/2/3 columns, processes and docker switch between table and card/list
 * renderers, and the panel width is draggable (320–900px).
...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function fmtPort(p) { return String(p).replace("->", " → "); }
    ...
    var CSS = [
      ...
      ".dsm-root{position:fixed;top:0;right:0;height:100vh;width:480px;min-width:320px;max-width:100vw;box-sizing:border-box;z-index:1500;display:flex;flex-direction:column;background:var(--dsw-specific-sidebar-fill,#17171e);border-left:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.08));box-shadow:-24px 0 48px rgba(0,0,0,.35);color:var(--dsw-alias-label-primary,#e7eaf0);font-family:var(--dsw-font-family,ui-sans-serif,system-ui,-apple-system,'Segoe UI',sans-serif);font-size:13px;line-height:1.5;animation:dsm-in .18s ease;container-type:inline-size}",
      ...
      "@container (min-width:380px){.dsm-metrics-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}",
      "@container (min-width:720px){.dsm-metrics-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}",
      ...
      ".dsm-proc-row{background:var(--dsw-alias-button-elevated-fill,rgba(255,255,255,.04));border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.07));border-radius:10px;padding:10px 12px}",
      ...
      ".dsm-container-state{font-size:11px;color:var(--dsw-alias-label-secondary,#9aa3b2);margin-left:auto;flex:none}",
      ...
      ".dsm-container-stat{display:flex;gap:6px}",
    ...
    // guard prevents overlapping requests when the host is slow.
    // ---------------------------------------------------------------------
    ...
    function ProcessesPanel(props) {
      ...
      return h("div", { className: "dsm-panel" },
        ...
        // narrow card list (<680px container)
        h("div", { className: "dsm-proc-list" },
          filtered.map(function (p) {
            return h("div", { className: "dsm-proc-row", key: p.pid },
    ...
    function DockerPanel(props) {
      ...
      function portChips(c) {
        ...
        return h("div", { className: "dsm-port-chips" },
          c.ports.map(function (p) { return h("span", { className: "dsm-port-chip", key: p }, fmtPort(p)); })
        );
      }
      ...
      return h("div", { className: "dsm-panel" },
        ...
        h("div", { className: "dsm-docker-table-wrap" },
          h("table", { className: "dsm-table" },
            ...
            h("tbody", null, containers.map(function (c) {
        ...
        // narrow container cards (<680px container)
          ...
          containers.map(function (c) {
            ...
            return h("div", { className: "dsm-container-card", key: c.id },
              h("div", { className: "dsm-container-head" },
                ...
                h("span", { className: "dsm-container-state", style: { color: stateColor } }, containerStateLabel(c.state))
              ),
              ...
              (c.ports && c.ports.length) ? h("div", { className: "dsm-port-chips" },
                c.ports.map(function (p) { return h("span", { className: "dsm-port-chip", key: p }, fmtPort(p)); })
              ) : null
            );
          })
    ...
    function MonitorDrawerBody(props) {
      ...
      // Independent error channels per RPC (overview / processes / docker).
      ...
      usePoll(function () {
        return callRpc(connection, "processes", {}).then(function (res) {
          if (res && res.ok) { setProcesses(res.value); setProcessError(null); }
      ...
      usePoll(function () {
        return callRpc(connection, "containers", { stats: tab === "docker" }).then(function (res) {
          if (res && res.ok) { setContainers(res.value); setDockerError(null); }
      ...
      if (tab === "overview") {
        ...
        body = h(DockerPanel, { data: containers, error: dockerError });
      }
