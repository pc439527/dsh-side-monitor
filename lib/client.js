  ...
  factory: function (require) {
    ...
    function formatContainerStatus(status) {
      ...
      if (m) {
        ...
        var unit = u.indexOf("second") === 0 ? "秒" : u.indexOf("minute") === 0 ? "分钟" : u.indexOf("hour") === 0 ? "小时" : "天";
        return "已运行 " + n + " " + unit;
      }
      if (/^Exited/.test(s)) return "已退出" + s.slice(6);
      if (/^Created/.test(s)) return "已创建";
      if (/^Restarting/.test(s)) return "重启中";
      if (/^Paused/.test(s)) return "已暂停";
      return s;
    }
    ...
    function MetricCard(props) {
      ...
      var sub = props.value == null ? "正在采样…" : props.sub;
    ...
    function SystemSection(overview) {
      ...
      if (overview.physicalCores != null) rows.push(["物理核心", String(overview.physicalCores)]);
      rows.push(["逻辑 CPU", String(overview.cpuCores)]);
      rows.push(["架构", overview.arch]);
      rows.push(["总内存", fmtBytes(overview.memoryTotal)]);
      return h("div", { className: "dsm-section" },
    ...
    function OverviewPanel(props) {
      ...
      var cpuSub = (phys != null && phys !== overview.cpuCores)
        ? phys + " 核 / " + overview.cpuCores + " 线程 · Load " + overview.load1.toFixed(2)
        : overview.cpuCores + " 逻辑 CPU · Load " + overview.load1.toFixed(2);
    ...
    function ProcessesPanel(props) {
      ...
      var countText = isGroup
        ? ((data.groupsTotal != null ? "共 " + data.groupsTotal + " 类 · " : "") + "匹配 " + data.matched + " 个")
        : ("共 " + data.total + " 个进程" + (query ? " · 匹配 " + data.matched + " 个" : ""));
      ...
      return h("div", { className: "dsm-panel" },
        ...
        h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 } },
          isGroup ? h("span", { className: "dsm-muted dsm-count" }, "按聚合 CPU 排序") : h("div", { className: "dsm-sortchips" },
          ...
          h("div", { className: "dsm-viewtoggle" },
            h("button", { className: view === "list" ? "dsm-view-active" : "", onClick: function () { setView("list"); } }, "列表"),
            h("button", { className: view === "group" ? "dsm-view-active" : "", onClick: function () { setView("group"); } }, "聚合")
          )
        ),
        isGroup
    ...
    function dedupPorts(ports) {
      ...
      (ports || []).forEach(function (p) {
        var key = (p.hostIp || "") + "|" + (p.hostPort != null ? p.hostPort : "") + "|" + (p.containerPort != null ? p.containerPort : "") + "|" + (p.protocol || "tcp");
        if (seen[key]) return;
        seen[key] = true;
        out.push(p);
      });
    ...
    function DockerPanel(props) {
      ...
      if (!data.available) {
        return h("div", { className: "dsm-panel" },
          h("div", { className: "dsm-state dsm-error", title: data.error || "" }, "Docker 不可用 · " + (data.error || "未检测到 /var/run/docker.sock"))
        );
      }
      ...
      function copyPort(p) {
        ...
        copyText(portHost(p) + ":" + p.hostPort, function (ok) { showToast(ok ? "已复制地址" : "复制失败"); });
      }

      function handlePortClick(p) {
        if (!isPublishedPort(p)) { showToast("未发布到宿主机，仅容器内可用"); return; }
        if (isLoopbackPort(p)) { showToast("仅宿主机本地可用"); return; }
        ...
        return h("div", { className: "dsm-port-chips" },
          ports.map(function (p, i) {
            if (!isPublishedPort(p)) {
              return h("span", {
                key: "u" + i,
                className: "dsm-port-chip dsm-port-chip-locked",
                title: "未发布到宿主机（仅容器内端口），不可打开",
              },
            ...
            if (isLoopbackPort(p)) {
              return h("span", {
                key: "l" + i,
                className: "dsm-port-chip dsm-port-chip-locked",
                title: "仅宿主机本地（" + portHost(p) + ":" + p.hostPort + "），浏览器无法从外部打开",
              },
                h("span", { className: "dsm-port-ico" }, "🔒"),
                String(p.hostPort)
              );
            ...
            var proto = portWebProtocol(p);
            var label = String(p.hostPort);
            var tooltip = "宿主 " + portHost(p) + ":" + p.hostPort + " · 容器端口 " + p.containerPort + "/" + (p.protocol || "tcp");
            return h("button", {
              key: String(i) + label,
      ...
      return h("div", { className: "dsm-panel" },
        ...
        portMenu ? h("div", { className: "dsm-portmenu-backdrop", onClick: function () { setPortMenu(null); } },
          h("div", { className: "dsm-portmenu", style: { left: portMenu.x, top: portMenu.y }, onClick: function (e) { e.stopPropagation(); } },
            ...
            isLoopbackPort(portMenu.port) ? null : h("button", { onClick: function () { openPort(portMenu.port, "http"); setPortMenu(null); } }, "HTTP 打开"),
            isLoopbackPort(portMenu.port) ? null : h("button", { onClick: function () { openPort(portMenu.port, "https"); setPortMenu(null); } }, "HTTPS 打开"),
            h("button", { onClick: function () { copyPort(portMenu.port); setPortMenu(null); } }, isLoopbackPort(portMenu.port) ? "复制地址（宿主机本地）" : "复制地址")
          )
        ) : null
      );
    }
    ...
    function SourceModal(props) {
      ...
      var hostOr = env.systemSource === "host" ? "宿主机" : "当前容器";
      var procOr = env.processSource === "host" ? "宿主机" : "当前容器";
      var modeLabel = env.mode === "container" ? "Container" : env.mode === "host" ? "Host" : "—";
      var dockerLabel = env.dockerSource === "host"
        ? "宿主机 · " + (sources.dockerSocket || "/var/run/docker.sock")
        : env.dockerSource === "unavailable" ? "不可用" : "—";
      return h("div", { className: "dsm-modal-backdrop", onClick: props.onClose },
        h("div", { className: "dsm-modal", onClick: function (e) { e.stopPropagation(); } },
          ...
          h("div", { className: "dsm-kv" },
            row("运行环境", modeLabel),
            row("负载 / 运行时长", hostOr + " · " + sources.loadavg + " / " + sources.uptime),
            row("CPU 核心 / 型号", hostOr + " · " + sources.cpuinfo),
            row("内核版本", hostOr + " · " + sources.osrelease),
            row("操作系统", hostOr + " · " + sources.osRelease),
            row("网络接口", hostOr + " · " + sources.netDev + " + " + sources.fibTrie),
            row("磁盘", hostOr + " · " + sources.mounts + " + " + sources.mountinfo),
            row("进程", procOr + " · " + sources.processes),
            row("Docker", dockerLabel)
          ),
          consistency.warnings && consistency.warnings.length
            ? h("div", { className: "dsm-consistency" },
                h("div", { className: "dsm-consistency-head" }, "⚠ 一致性自检"),
    ...
    function AboutModal(props) {
      ...
      return h("div", { className: "dsm-modal-backdrop", onClick: props.onClose },
        h("div", { className: "dsm-modal", onClick: function (e) { e.stopPropagation(); } },
          h("button", { className: "dsm-modal-close", onClick: props.onClose, "aria-label": "关闭" }, "×"),
          h("div", { className: "dsm-modal-title" }, "关于"),
          h("div", { className: "dsm-kv" },
            row("Browser (Client)", "v" + CLIENT_VERSION),
            row("Host", props.hostVersion ? "v" + props.hostVersion : "未知"),
            row("RPC Protocol", "v" + PROTOCOL_VERSION)
          ),
          props.mismatch ? h("div", { className: "dsm-consistency" },
            h("div", { className: "dsm-consistency-head" }, "⚠ 版本不一致"),
            h("div", { className: "dsm-consistency-item" }, "Host 与浏览器端协议不兼容，请重启 DeepSeek Harness 加载最新 Host 端")
          ) : null
        )
      );
    }
    ...
    function buildDiagnostic(overview, processes, containers) {
      ...
      L.push("Consistency: " + (env && env.consistency && env.consistency.warnings && env.consistency.warnings.length ? env.consistency.warnings.join("; ") : "OK"));
    ...
    function MonitorDrawerBody(props) {
      ...
      function statusInfo() {
        ...
        var source = src ? " · " + (src === "unavailable" ? "不可用" : sourceLabel(src) + "视角") : "";
      ...
      return h("div", { className: "dsm-root" + (isMobile ? " dsm-root-mobile" : ""), style: isMobile ? { width: "100%" } : { width: width + "px" } },
          ...
          h("div", { className: "dsm-header-actions" },
            ...
            h("div", { ref: menuRef, style: { position: "relative" } },
              ...
              menuOpen ? h("div", { className: "dsm-menu" },
                ...
                h("button", { className: "dsm-menu-item", onClick: function () { setMenuOpen(false); setAboutModal(true); } }, "关于"),
        ...
        versionMismatch ? h("div", { className: "dsm-version-banner", style: { margin: "8px 14px 0" } },
          "⚠ 系统监控组件版本不一致 · Browser v" + CLIENT_VERSION + " · Host " + (hostVersion ? "v" + hostVersion : "未知") + " · 请重启 DeepSeek Harness"
        ) : null,
        ...
        aboutModal ? h(AboutModal, { hostVersion: hostVersion, mismatch: versionMismatch, onClose: function () { setAboutModal(false); } }) : null
      );
    }
