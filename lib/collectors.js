...
export function createSystemCollector(config = {}) {
  ...
  function aggregateProcesses(rows, payload) {
    ...
    for (const r of matched) {
      ...
      if (!g) {
        g = { name: r.name, command: r.command, count: 0, cpu: 0, mem: 0, pids: [] }
        groups.set(key, g)
      }
      g.count++
      g.cpu += r.cpu
  ...
  return {
    async overview() {
      ...
      // must not spawn the netns probe.
      const hostNet = hostMetrics && procRoot === '/host/proc' ? await refreshHostNetNs() : null
